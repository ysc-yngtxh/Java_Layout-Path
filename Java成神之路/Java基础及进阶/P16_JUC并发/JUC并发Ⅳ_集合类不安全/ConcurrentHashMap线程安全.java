...
public class ConcurrentHashMap线程安全 {
     ...
     *     3、else：表示当前插入的数组位置有元素。就插入到链表或红黑树尾端。(1.8之后使用的是尾插法)
     *     4、putVal()方法执行完毕后，会调用addCount()方法
     *     5、addCount()方法会判断当前HashMap中的键值对数量是否大于阈值，如果大于则进行扩容为原来2的幂次数倍
     *
     *     假设两个线程A、B都在进行put操作，且put数据的key部分不同，只是hash函数计算出的插入下标是相同的。
     *     当线程A执行【第二个if判断为true后】由于时间片耗尽导致被挂起，
     *     而线程B得到时间片后也去执行 【第二个if判断为true】并在该下标处插入了元素，完成了正常的插入。
     *     然后线程A获得时间片，由于之前已经进行了hash碰撞的判断，所以此时不会再进行判断，而是直接进行插入。
     *     这就导致了线程B插入的数据被线程A覆盖了，造成了原本应该在同一个数组下标中形成链表结构的两组数据 如今却只有一条数据。
     ...
     *    在JDK 1.8中，ConcurrentHashMap放弃了之前版本中使用的锁分段技术，转而采用 数组+链表 或 数组+红黑树 的结构。
