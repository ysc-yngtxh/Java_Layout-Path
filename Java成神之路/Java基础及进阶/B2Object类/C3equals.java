...
public class C3equals {
    public static void main(String[] args) {
        ...
        System.out.println(t1==t2);     /*这里的t1和t2都是对象，对象其实就是内存地址x23456，所以t1,t2内存地址不相等，结果为false
		                                                              所以，有的时候为了比较两对象内容是否相等 ，就用到了equals重写类*/

        boolean g = t1.equals(t2);     //Time类对象t1引用方法equals，将Time类对象t2作为形参obj与t1进行内容比较，结果为true
        System.out.println(g);

        Time t3 = new Time(1997,4,29);   //t1内容不等于t3，所以结果为false
...
class Time{  //其实每个类都继承了Object,即extends Object
    ...
    //重写要求：返回值类型一样，形参列表一样，
    public boolean equals(Object obj){
        if(obj == null  || !(obj instanceof Time)){    //当obj为空值null时候，当程序执行到所在的方法体时候会出现空指针异常
            return false;                              //！表示obj不是Time类型
        }
