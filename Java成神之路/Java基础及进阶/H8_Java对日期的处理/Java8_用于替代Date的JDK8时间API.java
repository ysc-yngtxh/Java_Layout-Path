...
public class Java8_用于替代Date的JDK8时间API {
    public static void main(String[] args) {
        ...
        // 时间格式化SimpleDateFormat不是线程安全的类，在多线程环境下调用format()方法
        // 一个线程刚设置好time值，另外的一个线程马上把设置的time值给修改了，导致返回的格式化时间可能是错误的
        // 而使用DateTimeFormatter格式化时间是线程安全的
        System.out.println(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss SSS")));

        System.out.println("Instant = " + LocalDateTime.ofInstant(Instant.now(), ZoneId.systemDefault())
