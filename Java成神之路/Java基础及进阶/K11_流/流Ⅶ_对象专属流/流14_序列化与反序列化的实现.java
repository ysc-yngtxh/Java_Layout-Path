 ...
 *          例如：①、将 Student对象 作为新增数据写入Mysql中；
 *               ②、通过SpringMvc将Student对象转为Json返回给客户端
 *    反序列化：DeSerialize      将硬盘上的数据重新恢复到内存当中，恢复成Java对象
 *          例如：①、将Mysql中查询到的数据重新恢复到内存中，恢复成Student对象；
 *               ②、还有前端传过来的Json数据，转为Student对象
 *
 ...
 *          在 Student类 编译时JVM会自动生成序列化版本号--A，随后我将 Student类 进行序列化处理。
