...
public class A关于Volatile的详解 {
     ...
     *           也即当一条线程修改了共享变量的值，新值对于其他线程来说是可以立即得知的
     ...
     *    3、可以理解为：在有关键字 volatile 的共享变量在进行写操作上，会添加一个的加锁行为
     *    4、加锁和解锁是同一把锁
     */
    static int a = 0;
    ...
    static class Visibility {
        ...
        //    设：线程1、2同时执行，线程1执行完第①步后被挂起、线程2全部执行完，那么主存中num的值为1
        //       但线程1的工作内存中还是0，由于线程1之前已读取了num的值 = 0，执行 num++ 后再次将num的值刷新到主存 = 1
        //       即 num++ 执行了2次，但2次都是从0变为1，故num的值最终为1
