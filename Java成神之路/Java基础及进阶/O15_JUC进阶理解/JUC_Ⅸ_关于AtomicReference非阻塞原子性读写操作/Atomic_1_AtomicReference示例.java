package O15_JUC进阶理解.JUC_Ⅸ_关于AtomicReference非阻塞原子性读写操作;
...
public class Atomic_1_AtomicReference示例 {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //       (3)、线程加锁 lock
        //       (4)、最后将工作内存里将赋值后的结果 刷新到主内存的共享变量 debitCard 里
        //       (5)、线程解锁 unlock
        // 3、设：线程1、2同时执行，线程1执行完第(1)步后被挂起、线程2全部执行完，那么主存中debitCard的金额值为10
        //       但线程1的工作内存中还是0，由于线程1之前已读取了amount的值 = 0，执行 dc.getAmount()+10 后再次将amount的值刷新到主存 = 10
        //       即 dc.getAmount() + 10 执行了2次，但2次都是从0变为10，故有重复的打印金额
