 ...
 *          byte类型最大值：01111111【01111111 = 10000000-1 = 2^7-1 = 127】
 *          byte类型最小值：11111111【11111111 = -1*2^7 = -128】
 *          byte类型取值范围：【-128 ~ 127】
...
public class 数据类型及进制转换 {
	...
	public static void main(String[] args) {
        ...
        // Java 不直接提供无符号整数类型
        // 解析无符号32位最大值（4294967295），存储到int中（实际是-1，但按无符号处理）
        int unsignedInt = Integer.parseUnsignedInt("4294967295");
        System.out.println("有符号视角：" + unsignedInt); // 输出 -1
        System.out.println("无符号视角：" + Integer.toUnsignedString(unsignedInt)); // 输出 4294967295
        // 无符号比较：（4294967295）> 100，返回1
        System.out.println(Integer.compareUnsigned(unsignedInt, 100)); // 输出 1
        // 有符号比较：（-1）< 100，返回-1
        System.out.println(Integer.compare(unsignedInt, 100)); // 输出 -1
        ...
        //     还有一种简易的方式来计算一个负数的补码：
        //         1、从左到右，选择第一个1，保持不变，左边的全部变为0，右边的全部取反。
        //         例如：计算-6的补码
        //               6的二进制表示是： 0000 0110
        //               -6的补码则表示是：1111 1010
		...
		//              -1的补码 + 1的补码：1111 1111 + 0000 0001 = 0000 0000
		//         对于计算机来说：十进制转换为二进制的时候，负数的二进制是负数的补码表示。

		// 1、&（按位与）：如果两位都是1，那么结果是1，否则结果是0。
		int a1 = 60;     // 0011 1100 = 0+0+2²+2³ +2⁴+2⁵+0+0 = 0+0+4+8 +16+32+0 = 60
		int b1 = 13;     // 0000 1101 = 2⁰+0+2²+2³ +0+0+0+0  = 1+0+4+8 +0+0+0   = 13
