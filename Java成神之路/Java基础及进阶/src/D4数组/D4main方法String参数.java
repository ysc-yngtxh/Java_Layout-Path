...
public class D4main方法String参数 {
    //这个方法程序员负责写出来，JVM负责调用，JVM调用的时候一定会传一个String数组过来。
    public static void main(String[] args){

        System.out.println("JVM给传递过来的String数组参数，它这个数组的长度是？" + args.length);    //在不写入情况下结果为0
            //通过测试得出：args不是null，JVM默认传递过来的这个数组对象的长度默认为0.

       /*String[] args数组是留给用户的，用户可以在控制台上输入参数，这个参数自动会被转换为”String[] args“
         如果是在DOS命令窗口上，可以这样运行程序：javac D4main方法String数组4.java---->java D4main方法String数组4 abc def xyz
       ...
       */
        ...
        if (args.length!=2){
         ...
         if("admin".equals(username) && "123".equals(password)){    //*****相比于上面的写法，这种写法不会造成空指针异常
