...
一、单行处理函数（这里可以简单的理解为函数只处理单行字段，以此分类；那么后面就是分组函数或者叫聚集函数，意味处理多行字段，但是输出都是一行字段）
  Ⅰ、字符函数
     1、length 获取参数值的字节个数（每个英文一个字节；中文需要看编码，utf8编码一个中文3个字节）
        select LENGTH(name) from t_order;  -- 比如 name='游家纨绔‘ ，那么返回数据为 12; name='yjwk‘ ，那么返回数据为 4

     2、concat 拼接字符串
        [1]、select CONCAT(name, alias) from t_order;
             -- 比如 name='游家纨绔‘，alias='小游'，返回结果为 ‘游家纨绔小游’
        [2]、select CONCAT_WS("_", name, alias, status) from t_order;
             -- 比如 name='游家纨绔‘，alias='小游'，status='PLEASE_PAY' 返回结果为 ‘游家纨绔_小游_PLEASE_PAY’

     3.upper、lower 将字符变为大写/小写
        [1]、select LOWER(name) from t_order;  -- 比如 name='Yjwk‘ 返回结果为 ‘yjwk’
        [2]、select UPPER(name) from t_order;  -- 比如 name='Yjwk‘ 返回结果为 ‘YJWK’

     4.substr、substring 从下标pos开始往后截取字符输出，索引从1开始（两种写法都可）
        [1]、select SUBSTR(name, 1, 2) from t_order;     -- 比如 name='齐天大圣孙悟空‘ 返回结果为 ‘齐天’
        [2]、select SUBSTRING(name, 1, 2) from t_order;  -- 比如 name='齐天大圣孙悟空‘ 返回结果为 ‘齐天’

     5.instr 查询子串在主串中第一次出现的下标,找不到返回0
        select INSTR(name, '孙') from t_order;
        -- 比如 name='齐天大圣孙悟空‘ 返回结果为  5 ; 如果name='齐天大圣悟空‘ 返回结果为  0

     6.trim 去掉前后空格/或指定字符
        [1]、select TRIM(name) FROM t_order;  -- 去除前后空白
        [2]、select LTRIM(name) FROM t_order; -- 去除前导空白
        [3]、select RTRIM(name) FROM t_order; -- 去除尾随空白
        [4]、select TRIM(BOTH 'a' from name) from t_order  -- 去除前导和尾随的'a'字符【默认写法 TRIM('a' from subject)】
        [5]、select TRIM(LEADING 'a' from name) from t_order     -- 去除前导的'a'字符
        [6]、select TRIM(TRAILING 'a' from name) from t_order    -- 去除尾随的'a'字符
        [7]、select TRIM(BOTH 'a' from trim(name)) from t_order  -- 去除前后空白后，再去除前后的'a'字符

     7.lpad 用指定的字符实现左填充指定长度
        [1]、select LPAD(subject, 2, "*") from t_order
             -- 如果 subject=‘齐天大圣孙悟空’ 结果为： 齐天
        [2]、select LPAD(subject, 10, "*") from t_order
             -- 如果 subject=‘齐天大圣孙悟空’ 结果为： ***齐天大圣孙悟空

     8.rpad 用指定的字符实现右填充指定长度
        [1]、select RPAD(subject, 2, "*") from t_order
             -- 如果 subject=‘齐天大圣孙悟空’ 结果为： 齐天
        [2]、select RPAD(subject, 10, "*") from t_order
             -- 如果 subject=‘齐天大圣孙悟空’ 结果为： 齐天大圣孙悟空***

     9.replace 替换
       [1]、select REPLACE(subject, '操', "*") from t_order
            -- 如果 subject=‘中学生在操场上做早操’ 结果为： 中学生在*场上做早*
       [2]、update t_order set subject = REPLACE(subject, '曹', '*' ) where id=1;
            -- 如果 subject='曹操'  结果id=1的数据中： subject='曹*'

  Ⅱ、数学函数
     1.round 四舍五入
       select ROUND(price) from t_order;     -- 如果 price=123.556 结果为：124
       select ROUND(price, 2) from t_order;  -- 如果 price=123.556 结果为：123.56

     2.ceil 向上取整，返回 >= 该参数的最小整数（向上记住其实就是找比它大的整数就好）
       select CEIL(price) from t_order;  -- 如果 price=123.009 结果为：124; 如果 price=-1.009 结果为：-1

     3.floor 向下取整，返回 <= 该参数的最大整数
       select FLOOR(price) from t_order; -- 如果 price=123.009 结果为：123; 如果 price=-1.009 结果为：-2

     4.truncate 截断（其实就是截断小数点后面的几位，和round不同，简单的保留）
       select TRUNCATE(price, 2) from t_order;  -- 如果 price=123.556 结果为：124.55  区别于四舍五入，纯粹的截取

     5.mod取余
       select MOD(price, 3) from t_order;  -- 如果 price=10 结果为：1
