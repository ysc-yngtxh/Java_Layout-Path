...
public class WebSocketServer {
	 ...
	 * @param sid     获取 WebSocket 端点的路径参数 sid
	 */
	@OnOpen
	public void onOpen(Session session, @PathParam("sid") String sid) {
		// TODO 从 Session 的 UserProperties 获取 Cookie
		Map<String, String> cookieMap = (Map<String, String>) session.getUserProperties().get("cookies");
		if (MapUtils.isEmpty(cookieMap)) {
			log.warn("WebSocketServer.onOpen no cookies");
		}
		String cookie = cookieMap.get("LOGIN_COOKIE_NAME");
		// TODO 这里可以根据 cookie 做登录验证。。。。。。
