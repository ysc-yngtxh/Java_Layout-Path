...
class SpringBoot3ShardingSphereApplicationTests {
    ...
    /**
     * 在 Apache ShardingSphere 中，广播表是一种特殊的表类型，它存在于每一个分片数据源中，并且在这所有的数据库中拥有完全相同的表结构和数据。这意味着，当数据发生变化时，这个变化会被同步到所有的分片中，从而保持数据的一致性。
     *
     * 广播表的设计目的是为了处理那些数据量不大但是却需要频繁与其他大表进行关联查询的小表。这类表通常包含了一些基本的参考数据，例如字典表、状态码映射表、省份信息等。由于这类表的数据量较小，而且通常用于连接查询，因此不适合进行分片处理。通过使用广播表，可以避免因为多次查询不同分片中的相同小表而导致的额外开销。
     *
     * 广播表的特点：
