...
/**
 * @author 游家纨绔
 * @dateTime 2023-07-28 12:50
 * @apiNote TODO 测试类
 */
@SpringBootTest
public class ESApplicationTests {

    private final Logger log = LoggerFactory.getLogger(ESApplicationTests.class);
    ...
    void test() throws IOException, URISyntaxException {
        URL url = new URI("http://localhost:9200/shop/_search").toURL();
    ...
    void batchAddDocument() throws IOException {
        ElasticsearchClient elasticsearchClient = elasticsearchClient();
        // 1、使用 BulkOperation
        List<User> users = Arrays.asList(new User(1, "赵四", "20", 26, new Date()),
                new User(2, "阿旺", "25", 26, new Date()),
                new User(3, "刘菲", "22", 26, new Date()),
                new User(4, "冬梅", "20", 26, new Date()));
    ...
    void searchTwo() throws IOException {
        ...
        Query byMaxAge = RangeQuery.of(m -> m
                        .number(n -> n
                                        .field("age")
                                        // Elasticsearch 范围查询接受大范围的值类型。我们在这里创建最高价格表示。
                                        .gte(doubleMaxAge)
                        )
                )
                ._toQuery();
