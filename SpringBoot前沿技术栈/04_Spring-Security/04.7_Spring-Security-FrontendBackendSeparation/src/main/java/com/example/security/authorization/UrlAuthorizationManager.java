...
public class UrlAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
    ...
    @Override
    public @Nullable AuthorizationResult authorize(Supplier<? extends @Nullable Authentication> authentication, RequestAuthorizationContext requestAuthorizationContext) {
        // 获取当前用户访问路径
        ...
        if (!roleIds.isEmpty()) {
            ...
            isAuthorizationUrl = sysMenus.stream().anyMatch(item -> {
                PathPatternRequestMatcher pathMatcher = PathPatternRequestMatcher.withDefaults().matcher(HttpMethod.POST, item.getPath());
