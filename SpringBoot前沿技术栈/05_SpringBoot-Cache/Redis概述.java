 ...
 *               psubcribe news.*
 *
 * 十二、redis的主从复制(Master-Slave)：主少从多，主写从读，读写分离，主机写数据同步复制到从机
 *          搭配一主二从redis集群
 ...
 *           缺点：1、主节点故障时，需要手动切换到从节点，故障恢复时间较长。
 *                2、主节点承担所有写操作，可能成为性能瓶颈。
 *                3、无法实现数据分片，受单节点内存限制。
 *
 * 十三、redis哨兵模式(sentinel)：主机宕机，从机上位的自动版（参考资料：https://segmentfault.com/a/1190000039766545）
 *         1、搭建一主二从集群架构，支持（主写从读）
 *         2、提供哨兵配置文件：
 ...
 *                依然无法实现数据分片，受单节点内存限制。
 *
 * 十四、集群模式(Cluster)：Redis Cluster是Redis的分布式解决方案，支持自动分片和故障转移
 *         1、Cluster模式是Redis的一种高级集群模式，它通过数据分片和分布式存储实现了负载均衡和高可用性。
 *            在Cluster模式下，Redis将所有的键值对数据分散在多个节点上。每个节点负责一部分数据，称为槽位。
 *            通过对数据的分片，Cluster模式可以突破单节点的内存限制，实现更大规模的数据存储。
 *         2、注意⚠️：主节点（Master）处理所有读写请求，而从节点（Slave）仅作为数据备份和故障转移的备用节点，默认不参与读写。
 *                   因为主从复制是异步的，使用这种方式可以很好的解决主从节点的数据不一致问题，也能够提高系统的性能。
 *         2、数据分片与槽位
 *            Redis Cluster将数据分为16384（2^15）个槽位，每个节点负责管理一部分槽位。
 ...
 *            3.1、cluster节点端口号：port 7001
 *                 开启集群模式：cluster-enabled yes
 *                 节点超时时间：cluster-node-timeout 15000
 *            3.2、像这样的配置，一共需要创建6个，我们做一个三主三从的集群。
 ...
 *                一些复杂的多键操作可能受到限制
 *
 * 十五、缓存穿透：key对应的数据在数据源并不存在，每次针对此key的请求从缓存获取不到，请求都会到数据源，从而可能压垮数据源。
 ...
 *
 * 十八、Redis 的 大Key（BigKey） 和 热Key（HotKey）
 *      参考资料：https://developer.aliyun.com/article/1603836
 *
 * 十九、跳表结构
 ...
 *                [网络分区后]
 *                    分区1：主节点A（认为B、C下线）
 *                    分区2：从节点B+C（认为A下线，选举B为新主）
 *          结果：出现两个"主节点"同时接受写入，数据严重不一致
 *
 *          场景2：分布式锁服务的脑裂
 *                [正常情况]
 *                    客户端1 —— 持有锁 → Redis主节点
 *                    客户端2 —— 等待锁
 *                [脑裂发生后]
 *                    分区1：客户端1 + Redis旧主（锁仍显示被持有）
 *                    分区2：客户端2 + Redis新主（认为锁已释放）
 *          结果：两个客户端同时获得同一把锁，违反互斥性原则
 *
 *      脑裂的解决方案
 *          1、心跳链路高可用
 *             集群中出现网络原因，导致的是集群内foller和leader之间的通信问题，也就是互相感知不到了对方的存在foller感知不到leader存在就会进行选举。
 *             所以加强心跳通信的可靠性，比如通过多种方式进行心跳信息的传递，当有一条链路失效时，还有其他链路能够将心跳信息发出，避免出现多个分区。
 *             可以考虑主动发送心跳，被动拉取心跳，通过mq发送心跳等方式，总之是通过多种方式将心跳信息发出去，避免单链路故障导致多分区出现。
 *          2、设置合法选举数
 *             集群在进行leader选举时，一定要达到一定的选举数才能进行有效的选举，比如zookeeper中设置要超过原集群中半数以上的节点同意才能进行有效的选举。
 *             这样及时形成了两个分区，那也只有一个分区能够选举出新的leader，并且在两个分区中的通信问题修复后，新leader具有更高的version，可以通过epoch广播来将新的leader信息同步给旧的leader。
