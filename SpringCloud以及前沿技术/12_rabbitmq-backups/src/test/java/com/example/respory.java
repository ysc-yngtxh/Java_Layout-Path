...
public class respory {
    ...
    @RabbitListener(bindings = {  //bindings是交换机连接队列的规则
            @QueueBinding(
                    exchange = @Exchange(value = "logs",type= "fanout",durable="false"),//交换机的名称和类型和持久化
    ...
    @RabbitListener(bindings = {  //bindings是交换机连接队列的规则
            @QueueBinding(
                    exchange = @Exchange(value = "info",type= "direct"),//交换机的名称和类型
    ...
    //所以如果你不想被无端消费掉，就使用注入方法。不过这种就不能实时获取数据
    @Bean
    public Binding directBinding(){
        return BindingBuilder.bind(new org.springframework.amqp.core.Queue("yscs"))
                .to(new DirectExchange("ysc")).with("");
        /*
