...
public class Aspect4 {
    ...
    /** @Pointcut 定义和管理切入点，如果你的项目中有多个切入点表达式是重复的，可以使用@Pointcut <p>
     *        属性：value 切入点表达式。<p>
     *        位置：在自定义的方法上面。<p>
	...
	public Object MyText3(ProceedingJoinPoint pjp) throws Throwable {
		...
		System.out.println("然后我们再来看一下小曹的环绕通知，在目标方法执行之前：" + LocalDateTime.now().format(dateTimeFormatter));
	...
	// 当存在 Before、Around、AfterReturning 等通知时，执行顺序为：Around -> Before -> AfterReturning
	// 总结：当我们环绕通知没有返回值时，会影响到后置通知的 returning="res" 的参数值(为null)
	//      说明，在执行环绕通知时，把目标方法的返回值 '挪用' 到自己身上了。
	//      所以，后置通知上目标方法的返回值要看环绕通知的返回值的 '脸色'
}
