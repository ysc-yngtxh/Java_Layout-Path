...
public class MyController {
    ...
    /** POST请求方式：用户在输入中文时会出现乱码状况。<p>
     *     所以，在servlet里我们会选用这条语句让浏览器使用这条字符编码
     *         request.setCharacterEncoding("utf-8");
     *     但是在框架中，我们一般会在过滤器中进行修改字符编码
     */
    ...
    /** 请求中参数名和处理器方法的形参名不一样
     *  @RequestParam 解决请求中参数名形参名不一样的问题 <p>
     *     属性：<p>
     *          1、value：例如：用户请求的参数名(index.xml中提交的参数名--rname)与处理器方法的形参名(name)不一样会导致报错400
     *                        所以，这个时候就需要在value值上说明rname <p>
     *          2、require 是一个Boolean。required = false表示该参数可以不传，required在一个请求中默认值是为true。<p>
     *          3、true：表示请求中必须包含此参数。例如：：如果是get请求地址栏上要有请求参数(要不然会报错400) <p>
     *          4、400状态码：畸形的请求语法、无效的请求信息帧或者虚拟的请求路由 <p>
     *     位置：在处理器方法的形参定义的前面
     */
    ...
    /** 对象接收参数：<p>
     *    1、处理器方法形参是Java对象，这个对象的属性名和请求中参数名一样的 <p>
     *    2、框架会创建形参的Java对象，给属性赋值，请求中的参数是name，框架会调用setName() <p>
