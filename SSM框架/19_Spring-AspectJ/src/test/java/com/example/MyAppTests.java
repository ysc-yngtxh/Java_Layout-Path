...
public class MyAppTests {
    ...
    public void text10(){
        ...
        System.err.println("由于 AttrServiceImpl 被 Aop切面，所以这里获取的是代理对象。" +
                "\n而原始对象 AttrServiceImpl 中的属性 injectService 在代理对象中值为 null" +
                "\n因为代理对象并没有对任何属性进行依赖注入，因此自然为null.可以通过打断点的方式，在控制台查看代理对象的属性数据。\n"
                + proxy.getClass().getName());
        proxy.doSomething();
        System.err.println("这里你肯定会很奇怪：为什么通过代理对象调用的方法中打印的属性却有值？\n" +
                "因为代理对象可以看做是继承目标对象的一个新的类，" +
                "class proxy extends AttrServiceImpl {\n" +
