 ...
 *
 *          a、RDB策略：在指定时间间隔内，redis服务执行指定次数的写操作，会自动触发一次持久化操作。
 ...
 *          b、AOF策略：采用操作日志来记录进行每一次写操作，每次redis服务启动时，都会重新执行一遍操作日志中的指令。
 ...
 *          4、discard:清除所有压入队列中的命令，并且结束整个事务
 *          5、watch:监控某个键，当事务在执行过程中，此键代码的值发生变化，则本事务放弃执行；否则正常执行
 *
 *                比如付款的时候，避免两个人在同一账户掐同一时间中付款。我们会监控某一个键，来进行监控事务正常执行。
 *               id balance version
