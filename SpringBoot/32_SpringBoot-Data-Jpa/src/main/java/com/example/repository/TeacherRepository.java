...
public interface TeacherRepository extends JpaRepository<Teacher, Integer> {
    ...
    @Query(nativeQuery = true, value = "select * from db_teacher c where c.name = :name")
    List<Teacher> findTeacher2(@Param("name") String name);

	// 正常使用，只是多加了一个 sort 参数而已
	@Query("select t from Teacher t where t.age = :age")
	List<Teacher> getTeachers(@Param("age") Integer age, Sort sort);
}
