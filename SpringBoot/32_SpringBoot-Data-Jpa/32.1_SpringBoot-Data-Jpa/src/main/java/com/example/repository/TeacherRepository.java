...
public interface TeacherRepository extends JpaRepository<Teacher, Integer> {
	...
	// 在 JPQL 中使用 SpEL 表达式（原生Sql 不具备 SpEL能力）
	// 示例 1: 直接使用方法参数
	@Query("select t from Teacher t where t.email = :email AND t.status = :#{#status}")
	List<Teacher> findByEmailAndStatus(@Param("email") String email, String status);
