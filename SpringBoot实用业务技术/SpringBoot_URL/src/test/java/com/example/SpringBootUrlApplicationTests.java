...
class SpringBootUrlApplicationTests {
	...
	//       URLClassLoader 会首先尝试让其默认的父类加载器从应用程序的类路径中加载类（双亲委派机制）。
	...
	public void contextLoads2() throws MalformedURLException, ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
		...
		try (URLClassLoader loader1 = new URLClassLoader(new URL[]{url});
			...
			// 按理来说：URL 的地址跟加载的全限定名称路径并不能完美匹配上（缺少/target/classes）,并且类加载器只能加载 class 文件，不能加载 java 文件。
		...
		Class<?> clazz2 = loaderAndNull.loadClass("com.example.pojo.Hello");
		System.out.println("使用 URLClassLoader(new URL[]{url}, null)，当前类加载器：" + clazz2.getClassLoader());
		System.out.println("使用 URLClassLoader(new URL[]{url}, null)，父类加载器：" + clazz2.getClassLoader().getParent());
		clazz2.getDeclaredConstructor().newInstance();
	}

	// TODO 自定义类加载器
	@Test
	public void contextLoads3() throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException, ClassNotFoundException, MalformedURLException {
		URL url = new File(System.getProperty("user.dir")).toURI().toURL();
		...
		Class<?> clazz = customClassLoader.loadClass("com.example.pojo.Hello");
		System.out.println("使用自定义的加载器调用loadClass()方法，当前类加载器：" + clazz.getClassLoader());
		System.out.println("使用自定义的加载器loadClass()方法，其父类加载器：" + clazz.getClassLoader().getParent());

		// 通过自定义的加载器调用 findClass() 方法，使用的自定义的加载逻辑
		Class<?> clazz1 = customClassLoader.findClass("com.example.pojo.Hello");
		System.out.println("使用自定义的加载器findClass()方法，当前类加载器：" + clazz1.getClassLoader());
		System.out.println("使用自定义的加载器findClass()方法，其父类加载器：" + clazz1.getClassLoader().getParent());
