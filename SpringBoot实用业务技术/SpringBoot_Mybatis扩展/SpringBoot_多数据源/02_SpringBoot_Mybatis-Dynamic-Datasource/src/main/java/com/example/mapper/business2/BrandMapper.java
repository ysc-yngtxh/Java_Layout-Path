...
/**
 * 品牌表，一个品牌下有多个商品（spu），一对多关系(Brand)表数据库访问层
 *
 * @author 游家纨绔
 * @since 2023-09-02 14:50:20
 */
public interface BrandMapper {

	/**
	 * @Results用法 当数据库字段名与实体类对应的属性名不一致时，可以使用@Results映射来将其对应起来。
	 * column为数据库字段名，property为实体类属性名，jdbcType为数据库字段数据类型，id为是否为主键。
	 */
	@Select("select * from business2.brand")
	...
	/**
	 * @ResultMap用法 当这段@Results代码需要在多个方法用到时，为了提高代码复用性，
	 * 我们可以为这个@Results注解设置id，然后使用@ResultMap注解来复用这段代码。
	 */
	@Select("select * from business2.brand where name=#{name}")
	...
	@Insert("insert into business2.brand(name, image, letter) values(#{brand.name}, #{brand.image}, #{brand.letter})")
	void insertNameAndLetter(@Param("brand") Brand brand);
	...
	@Insert("insert into business2.brand(name, image, letter) values(#{brand.name}, #{brand.image}, #{brand.letter})")
	void insertNameAndSelectKey(@Param("brand") Brand brand);
}
