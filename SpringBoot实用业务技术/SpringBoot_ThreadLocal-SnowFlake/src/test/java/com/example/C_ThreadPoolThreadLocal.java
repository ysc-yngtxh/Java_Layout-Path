...
public class C_ThreadPoolThreadLocal {
    ...
    // TODO 1、现在需要实现这样一个功能，在一个线程执行过程中开启了一个Scope，随后使用线程池执行任务，要求在线程池中也能获取当前Scope中的状态数据。
    ...
    //      4、解决方法：在提交runnable时，将当前的Scope引用存到runnable对象中，当获得线程执行时，
    //                 将Scope替换到执行线程中，执行完成后，再恢复现场。
    @Test
