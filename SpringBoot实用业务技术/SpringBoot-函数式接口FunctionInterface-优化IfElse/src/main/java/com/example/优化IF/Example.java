...
import com.example.优化IF.函数式接口加Map.QueryGrantTypeService;
import com.example.优化IF.策略模式.Context;
import com.example.优化IF.策略模式.RedPaper;
import com.example.优化IF.策略模式.Shopping;
import com.example.优化IF.策略模式加工厂加单例模式.VipRechargeFactory;
import com.example.优化IF.策略模式加工厂加单例模式.VipRechargeStrategy;
...
public class Example implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 方法三：策略模式通过接口、实现类、逻辑分派来完成，把 if语句块的逻辑抽出来写成一个类，更好维护。
        ...
        // 方法四：其实与方法三有异曲同工之妙，但还是提供思路作为参考
        VipRechargeStrategy strategy =
                VipRechargeFactory.getInstance().getConcurrentStrategy(rechargeType);
        if (strategy == null)
            // "策略不符合，及时中断"
            return;
