 ...
 *            max_limit_id:   表示在生成ReadView时，系统中应该分配给下一个事务的id值，即m_ids中的id最大值 + 1
 ...
 *                                     +--------+--------+--------------+-----+------+
 *          ⑤、提交事务B
 *          ⑥、事务A第二次执行查询操作，生成一个Read View副本（RR隔离级别下，同一事务从始至终只有一个ReadView）
 *                                     +----------------+---------+
 *                                     | m_ids          | 100,101 |
 *                                     +----------------+---------+
 *                                     | max_limit_id   | 102     |
 ...
 *                由于 事务B 已经提交，事务A 中的 ReadView 也并不能干预其执行 UPDATE或DELETE 语句来修改新插入的 id=4 记录。
 *                并且 事务A 修改 id=4 的这条数据行在此时是没有任何锁机制的，所以 事务A 在修改数据行时添加排他锁不会有任何阻塞。
 *                那么 id=4 的这条数据就会生成一个新的版本（旧数据存入 undo log，形成 Undo log 版本链）。
 *                这样一来，这条新版本的记录的隐藏列 trx_id值 就被设置为当前事务的 ID（trx_id=100）。
