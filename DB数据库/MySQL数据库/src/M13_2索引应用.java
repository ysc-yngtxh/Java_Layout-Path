...
4、在Mysql中，什么是回表，什么是索引覆盖，索引下推？
   ...
   [3]、索引下推：
        ...
        需求：检索出表中名字第一个字是'张'，而且年龄是10岁的所有用户。SELECT * FROM t_user WHERE name LIKE '张%' AND age=10;
             在MySQL 5.6之前，是没有使用索引下推(ICP)，存储引擎通过 '联合索引' 找到满足 [name like '张%'] 条件叶子节点中的主键id，
             并通过主键id逐一进行回表扫描，然后通过 '聚集索引' 找到完整的行记录，最后根据 [age=10] 条件进行结果筛选。
             但是这样我们就把 '联合索引' 的另一个字段 [age] 浪费了。
             所以在MySQL 5.6之后，索引下推默认是开启的。存储引擎通过 '联合索引' 找到满足 [name like '张%'] 条件叶子节点索引列数据，
             由于 '联合索引' 中包含 [age] 列，所以存储引擎直接在叶子节点索引列里按照 [age=10] 过滤，按照过滤后的数据再一一进行回表扫描。
---------------------------------------------------------------------------------------------------------------
5、Mysql 索引如何进行优化
