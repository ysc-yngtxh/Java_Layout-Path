...
---------------------------------------------------------------------------------------------------------------
5、在Mysql中，什么是回表，什么是覆盖索引，索引下推？
---------------------------------------------------------------------------------------------------------------
6、Mysql 聚集(聚簇)索引 和 非聚集(非聚簇)索引 的区别
...
---------------------------------------------------------------------------------------------------------------
7、Mysql的组合索引的结构是什么样的
---------------------------------------------------------------------------------------------------------------
8、Mysql 索引如何进行优化
   [1]、避免过度索引：过多的索引会增加数据库的存储和维护成本，同时也会影响数据库的性能。因此，在建立索引时，应该根据实际查询场景进行决定，尽量避免过度索引。
   [2]、建立复合索引：对于经常需要同时查询多个列的语句，建立复合索引可以有效地提高查询效率。需要注意的是，复合索引的建立顺序需要根据实际查询场景进行决定。
   [3]、使用覆盖索引：覆盖索引是指查询语句只需要使用索引就可以获取需要的数据，而不需要回表操作。覆盖索引可以减少MySQL的I/O操作，从而提高查询效率。
   [4]、避免使用函数在索引列上进行计算：在查询语句中，应尽量避免在索引列上使用函数进行计算，因为这样会使MySQL无法使用索引，从而需要进行全表扫描。
   [5]、定期维护索引：定期维护索引可以清理无用的索引，以保证数据库的正常运行。例如，可以使用OPTIMIZE TABLE语句来对表进行优化，以清理无用的索引和碎片。
 */
