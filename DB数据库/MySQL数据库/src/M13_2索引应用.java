...
3、Mysql 聚集(聚簇)索引 和 非聚集(非聚簇)索引 的区别
   ...
   总结：一级索引(聚集索引) 存储在 'B+树' 中叶子节点的数据，相当于表中对应索引值的一行数据
        二级索引(非聚集索引) 存储在 'B+树' 中叶子节点的数据，相当于表中对应索引值的一个列数据（还包含一个主键值）
---------------------------------------------------------------------------------------------------------------
4、在Mysql中，什么是回表，什么是索引覆盖，索引下推？
   [1]、回表：
            回表是发生在 '二级索引' 上的一种数据查询操作，简单点讲就是我们要查询的列不在 '二级索引' 的列中，
        那么必须根据 '二级索引' 查到主键ID，然后再根据主键ID到 '聚簇索引树' 上去查询整行的数据，这一过程就叫作 ---- 回表。
   ...
   [2]、索引覆盖：
            当SQL语句中查询的列都在索引中时，我们就不需要通过 回表 去把整行数据都捞出来了，可以从 '非聚簇索引树' 中直接获取到我们需要的列数据，
        这就叫 ---- 索引覆盖。简单点来讲就是：所有不需要 '回表' 的查询操作都叫 '索引覆盖'。
   ...
   [3]、索引下推：
        ...
        需求：检索出表中名字第一个字是'张'，而且年龄是10岁的所有用户。SELECT * FROM t_user WHERE name LIKE '张%' AND age=10;
        ①、在MySQL 5.6之前，是没有使用索引下推(ICP)，存储引擎通过 '联合索引' 找到满足 [name like '张%'] 条件叶子节点索引列数据，
            假如叶子节点中的满足 [name like '张%'] 条件的数据是： [id=1,name='张三',age=10]、 [id=4,name='李四',age=20]。
            并通过主键id逐一进行回表扫描，然后通过 '聚集索引' 找到完整的行记录，最后根据 [age=10] 条件进行行记录筛选。
            这样我们总共需要进行两次回表，但是这样就把 '联合索引' 的另一个字段 [age] 浪费了，毕竟都没发挥出这个索引性能。

        ②、在MySQL 5.6之后，索引下推(ICP)是默认开启的。存储引擎通过 '联合索引' 找到满足 [name like '张%'] 条件叶子节点索引列数据，
            假如叶子节点中的满足 [name like '张%'] 条件的数据是： [id=1,name='张三',age=10]、 [id=4,name='李四',age=20]。
            由于 '联合索引' 中包含 [age] 列，所以存储引擎直接在叶子节点索引列里按照 [age=10] 过滤，
            叶子节点中的满足 [name like '张%' AND age=10] 条件的数据是：[id=1,name='张三',age=10]，
            按照过滤后的数据再进行回表扫描，这样我们总共就只需要进行一次回表。
---------------------------------------------------------------------------------------------------------------
5、Mysql 索引如何进行优化
