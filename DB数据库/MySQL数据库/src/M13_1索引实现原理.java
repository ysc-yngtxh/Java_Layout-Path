...
---------------------------------------------------------------------------------------------------------------
5、B树（B-Tree）
   [1]、如果用B-Tree作为索引的数据结构，如图，可以在一个节点上存储更多的元素，元素数据为 k-v键值对 的形式。
        key 就是索引字段，data 就是索引字段所在的那一行的数据或是那一行数据所在的磁盘文件地址（或叫指针）。
        这种B树数据结构查找元素不是每次 Load 单个元素，而是把一个大的节点数据全部 Load 到内存，然后再在内存里再去比对，内存里操作比较快捷。

比如：我们要查找49这个元素，实际上是从根节点开始查找的，它一次性将根节点这个大节点一次性Load到内存里，然后用要查找的元素去比对。
49大于15小于56，在15和56之间有一个节点存储的是下一个节点的磁盘地址，指向下一个节点（这个节点的索引都是大于15小于56的），
然后再将这个节点一次性Load到内存去找这个元素，最终就找到了。
...
你想一下，前面我们说了一个节点不能太大也不能太小，就是16K，把data元素挪走以后，是不是这个节点就能存更多的冗余索引了，意味着分叉就更多了，分叉多叶子节点就多，意味着叶子节点就能存储更多的数据了。
            ...
            +----------+----⬇︎---+-----------+---⬇︎----+----------+----⬇︎----+      +---------+        +---------+
             ...
             ⬇︎                          ⬇︎                            ⬇︎
   +---------⬇︎--------+       +---------⬇︎----------+       +---------⬇︎---------+
   |   15    |    18   |➡︎ ➡︎ ➡︎ |   20    |    30    |➡︎ ➡︎ ➡︎ |    49    |   50    |➡︎ ➡︎ ➡︎
   +---------+---------+       +---------+----------+       +----------+---------+
