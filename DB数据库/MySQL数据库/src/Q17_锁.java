 ...
 *           特征：锁冲突概率低，并发性高，但是会有死锁的情况出现(比如：事务A：更新记录1 → 尝试更新记录2；事务B：更新记录2 → 尝试更新记录1。但是彼此交叉进行就会出现死锁)
 *
 ...
 *           7  | 商品D   | 40
 *           9  | 商品E   | 50
 *      [5]、记录锁(Record Lock)：属于行锁中的一种，是指对表中某一行记录进行锁定的机制（只会锁定一行记录，锁多行数据用的锁肯定不是记录锁）
 *                              '记录锁' 的粒度较小，在并发操作时，记录锁的获取和释放可能会导致性能问题。
 *           例如：SELECT * FROM products WHERE id = 5 FOR UPDATE; 会锁定id=5的记录。
 *           特征：当一个事务对表中的某一行记录进行当前读或修改时，会为该行数据添加 '记录锁'，其他事务如果要对同一行记录进行操作，必须等待记录锁被释放。
 *                '记录锁' 可以防止数据的一致性问题，确保在一个事务执行过程中，其他事务不能修改同一行记录。避免查询时被修改的不可重复读问题，
 *                也避免了事务在修改后但未提交被其他事务读取的脏读问题。
 *
 *      [6]、间隙锁(Gap Lock)：属于行锁中的一种，'间隙锁'是在事务加锁后锁住满足条件的索引列空隙区间，防止其他事务在这个范围中插入新的行,
 *                            但是可以修改数据。很明显可以锁住多行数据。
 *           例如：SELECT * FROM products WHERE price BETWEEN 21 AND 39 FOR UPDATE;
 *                该语句添加 '排他锁' 的方式使用 '间隙锁'，锁定(20,40)区间（price=20和40不被锁定），避免其他事务对这个区间数据进行新增。
 *           特殊情况：SELECT * FROM products WHERE price = 25 FOR UPDATE;
 *                由于 price=25 的数据不存在，所以会向 25 的前后两边数据发散，锁定 price(20,30) 区间。
 *           特征：事务并发的时候，如果没有间隙锁，就会发生幻读问题，在同一个事务里，事务的两次查询出的结果会不一样。
 *                间隙锁只会出现在 REPEATABLE_READ(可重复读) 的事务级别中。
 *
 *      [7]、临键锁(Next-Key Lock)：属于行锁的一种，就是 '记录锁' 和 '间隙锁' 的组合，'临键锁'会把查询出来的记录锁住，
 *                                同时也会把该范围查询内的所有间隙空间也会锁住，防止其他事务在这个范围中插入新的行或者更新已经存在的行。
 *           例如：SELECT * FROM products WHERE price > 20 AND price < 40 FOR UPDATE;
 *                该语句添加 '排他锁' 的方式使用 '临键锁'，按照左开右闭区间(前间隙+记录)，锁定 (20,30] 和 (30,40] 区间，避免其他事务对这个区间数据进行新增或修改。
 ...
 *                            想象一下，版本号每次只会有一个用户扣减成功，不可避免的人为造成失败。这种时候就需要我们的第二种场景的乐观锁方法
 *
