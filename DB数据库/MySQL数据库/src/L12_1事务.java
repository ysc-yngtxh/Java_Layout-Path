...
3、关于事务之间的隔离性（理论上隔离级别包括4个）
     ...
     第三级别：可重复读(repeatable read)
             ...
             [5]、小曹这时候又会问：为了维护可重复读，通过持有共享锁的方式不是不允许修改数据吗？怎么还能新增数据呢？
                  因为这个共享锁锁的是我们读取的数据行，读取的这些数据行是不允许进行修改的，但是不影响我往表中新增数据
             [6]、如何避免幻读：MVCC、记录锁、间隙锁
                  MySQL InnoDB存储引擎很大程度上避免幻读现象，避免方案有两种，但没有完全解决幻读问题：
