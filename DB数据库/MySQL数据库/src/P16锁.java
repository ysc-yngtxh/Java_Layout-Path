...
共享锁: 指的是一个事务在一行数据加上共享锁后，其他事务只能在其上加共享锁，互相不会阻塞，但不能添加排他锁
       就是多个事务只能读数据不能改数据，包括当前添加共享锁的事务只能读不能修改。
排他锁: 指的是一个事务在一行数据加上排他锁后，其他事务不能再在其上加其他的锁。
       当前添加排他锁的事务可以读也能修改，但是其他的事务只能读取不能修改
       (其他事务普通查询 SELECT语句 自动为查询的行添加共享锁，但是添加排他锁的事务会阻止其他事务对同一数据添加共享锁。因此这时候的查询就变成了普通无锁查询)
 */
public class P16锁 {
}
