...
2、MySQL从锁的粒度上分类
     ...
     [4]、记录锁(Record Lock)：属于行锁中的一种，是指对表中某一行记录进行锁定的机制，且只会锁定一行记录，不会锁住多行记录。
                             如果需要锁多行数据用的锁肯定不是记录锁。'记录锁' 的粒度较小，在并发操作时，记录锁的获取和释放可能会导致性能问题。
     ...
     [5]、间隙锁(Gap Lock)：属于行锁中的一种，'间隙锁'是在事务加锁后锁住满足条件的索引列空隙区间，遵循左开右闭原则,防止其他事务插入新的行。
          例如：select * from t_user where id>1 and id<4 for update;
               这个SQL语句添加 '排他锁' 的方式使用 '间隙锁'，锁住间隙区间 [1,4}，避免其他事务对这个区间数据进行新增或修改。
     ...
     [6]、临键锁(Next-Key Lock)：属于行锁的一种，就是 '记录锁' 和 '间隙锁' 的组合，'临键锁'会把查询出来的记录锁住，
                               同时也会把该范围查询内的所有间隙空间也会锁住，再之它会把相邻的下一个区间也会锁住。
          例如：select * from t_user where id>1 and id<=13 for update;
               会锁住id为 [1,5,10] 的记录；同时会锁住 [1——5}、[5——10}、[10——15} 的区间。
          特征：结合记录锁和间隙锁的特性，临键锁避免了在范围查询时出现脏读、重复读、幻读问题。
