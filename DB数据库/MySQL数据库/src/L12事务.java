...
3、什么是 MVCC？
      MVCC，即Multi-Version  Concurrency Control （多版本并发控制）。它是一种并发控制的方法，一般在数据库管理系统中，
...
MySQL的普通读被称为快照读是因为它是基于数据库的一个一致性快照来实现的。一致性快照读取的是查询开始时数据库中的数据，并且保证在整个查询过程中不受其他事务的修改影响。
快照读是通过在读取操作开始之前创建一个数据库的一致性快照来实现的。这个快照包含了所有已提交的事务在读取开始之前的数据。当进行读取操作时，MySQL引擎会使用该快照来保证读取的一致性。
相比之下，另一种读取方式是锁定读，它会在读取操作期间对相关数据进行锁定，以防止其他事务的并发修改。然而，锁定读可能会对并发性能产生一些影响。
因此，MySQL默认使用快照读来提供更好的并发性能和一致性。但需要注意的是，有些特定的读操作需要使用锁定读来确保读取的准确性，这时可以使用SELECT … FOR UPDATE和SELECT … LOCK IN SHARE MODE语句来实现。
