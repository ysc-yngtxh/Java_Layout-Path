...
Tomcat根据Servlet规范调用servlet接口实现类规则：
          1、Tomcat有权创建servlet接口实现类实例对象    Servlet com.bjpowernode.controller.A1Servlet = new com.bjpowernode.controller.A1Servlet();
          2、Tomcat根据实例对象调用Service方法处理当前请求   com.bjpowernode.controller.A1Servlet.service()

          com.bjpowernode.controller.A1Servlet---------->(abstract)HttpServlet---------->(abstract)GenericServlet------------>servlet接口
                   (extends)                   (extends)    init              (implements)                        (implements)
                                                            getServletConfig
                                                            getServletInfo
                                                            destory
 */
public class A1Servlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("A1Servlet类针对浏览器发送Get请求方式处理");
    }

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("A1Servlet类针对浏览器发送Post请求方式处理");
    }
}
